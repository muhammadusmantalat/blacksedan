                        ...
                        <div class="card">
                            ...
                            <div class="card-body  table-responsive">
                                ...
                                <table class="responsive table table-striped table-bordered" id="table_id_events">
                                    ...
                                    <tbody>
                                        @foreach ($accountRequests as $data)
                                            <tr>
                                                ...
                                                <td class="d-flex ml-4">
                                                        <form
                                                        action="{{ route('chauffer-requests.status', $data->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('POST')
                                                        <select name="status" class="form-control"
                                                            ...
                                                            <option value="0"
                                                                {{ $data->request_status == 0 ? 'selected' : '' }} disabled>
                                                                Pending</option>
                                                            <option value="1"
                                                                {{ $data->request_status == 1 ? 'selected' : '' }}>Accepted
                                                            </option>
                                                            <option value="2"
                                                                {{ $data->request_status == 2 ? 'selected' : '' }}>Rejected
                                                            </option>
                                                        </select>
                                                    </form>
                                                </td>
...
@section('js')
    ...
    function handleStatusChange(selectElement, productId) {
                if (selectElement.value == 2) {
                    $('#deactivationForm').attr('action', '{{ url('admin/requests/status/') }}/' + productId);
